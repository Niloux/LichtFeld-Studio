...
namespace {
    ...
    std::expected<std::tuple<ParseResult, std::function<void()>>, std::string> parse_arguments(
        ...
        try {
            ...
            constexpr auto init_help = "Initialize from splat file (.ply, .sog, .ssog, .spz, .usd, .usda, .usdc, .usdz, .resume)";
            constexpr auto export_help = "Also export the final trained splat next to project.licht: comma-separated ply, sog, ssog, spz, usd, usda, usdc, html, rad";
#endif
            ...
            ::args::ValueFlag<std::string> view_ply(mode_group, "path", "View file(s). Supports projects (.licht), splat (.ply, .sog, .ssog, .spz, .rad, .usd, .usda, .usdc, .usdz) and mesh (.obj, .fbx, .gltf, .glb, .stl) formats. If directory, loads all.", {'v', "view"}, studio_help);
